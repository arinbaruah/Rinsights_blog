...
```{r image, out.height = '300px', out.width= '800px', echo = FALSE,fig.cap='Source: www.ondeck.com'}
...
- The current dataset contains a total of __`r length(names(df_final))`__ variables.
- A total of __`r dim(df_final)[1]`__ observations were recorded in the dataset.

 2. The image \@ref(fig:inline) shows the inline r codes that were used to obtain the above results.

```{r inline, out.height = '50px', out.width= '1000px', echo = FALSE,fig.cap="Inline R codes"}
...
head(df_clean) %>% 
  DT::datatable(width = 1000, height = 450, options = list(pageLength = 5)) #Transforms a dataframe to an intuitive datatable.
...
```{r plot1, fig.cap='Relation between annual working hours and GDP per capita'}
...
```{r plot2, fig.cap='Productivity: Gross Domestic Produce per hour of work (in $/hr)'}
